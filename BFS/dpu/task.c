...
int main()
{
	...
	struct DPUParams* params_w = (struct DPUParams*)
	...
	if (numNodes > 0) {
		...
		for (uint32_t node = taskletNodesStart;
			...
			if (isSet(currentFrontierTile, node % 64)) { // If the node is in the current frontier
				// Visit its neighbors
				uint32_t nodePtr = load4B(nodePtrs_m, node,
				                       cache_w)
				    - nodePtrsOffset;
				...
				for (uint32_t i = nodePtr; i < nextNodePtr; ++i) {
					...
					uint64_t visitedTile = load8B(visited_m, neighborTileIdx,
					    cache_w);
					if (!isSet(visitedTile, neighbor % 64)) { // Neighbor not previously visited
						...
						uint64_t nextFrontierTile = load8B(nextFrontier_m,
						    neighborTileIdx,
						    cache_w);
