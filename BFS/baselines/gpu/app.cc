...
int main(int argc, char** argv)
{
	...
	cudaMalloc((void**)&csrGraph_d.nodePtrs,
	    (csrGraph_d.numNodes + 1) * sizeof(uint32_t));
	cudaMalloc((void**)&csrGraph_d.neighborIdxs,
	    csrGraph_d.numEdges * sizeof(uint32_t));
	uint32_t* nodeLevel_d;
	cudaMalloc((void**)&nodeLevel_d,
	    csrGraph_d.numNodes * sizeof(uint32_t));
	uint32_t* buffer1_d;
	...
	// Copy data to GPU
	cudaMemcpy(csrGraph_d.nodePtrs, csrGraph.nodePtrs,
	    (csrGraph_d.numNodes + 1) * sizeof(uint32_t),
	    cudaMemcpyHostToDevice);
	cudaMemcpy(csrGraph_d.neighborIdxs, csrGraph.neighborIdxs,
	    csrGraph_d.numEdges * sizeof(uint32_t),
	    cudaMemcpyHostToDevice);
	nodeLevel_gpu[srcNode] = 0;
	cudaMemcpy(nodeLevel_d, nodeLevel_gpu,
	    csrGraph_d.numNodes * sizeof(uint32_t),
	    cudaMemcpyHostToDevice);
	cudaMemcpy(prevFrontier_d, &srcNode, sizeof(uint32_t),
	    cudaMemcpyHostToDevice);
	...
	for (uint32_t level = 1; numPrevFrontier > 0; ++level) {
		...
		bfs_kernel(numBlocks, numThreadsPerBlock, csrGraph_d,
		    nodeLevel_d, prevFrontier_d, currFrontier_d,
		...
		currFrontier_d = tmp;
		cudaMemcpy(&numPrevFrontier, numCurrFrontier_d,
	...
	// Copy data from GPU
	cudaMemcpy(nodeLevel_gpu, nodeLevel_d,
	    csrGraph_d.numNodes * sizeof(uint32_t),
	...
	for (uint32_t i = 0; i < csrGraph.numNodes; ++i) {
		if (nodeLevel_cpu[i] != nodeLevel_gpu[i]) {
			printf("Mismatch detected at node %u (CPU result = %u, "
			       "GPU result = %u)\n",
