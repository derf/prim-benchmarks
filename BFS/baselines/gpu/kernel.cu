...
__global__ void _bfs_kernel(CSRGraph csrGraph, uint32_t* nodeLevel, uint32_t* prevFrontier, uint32_t* currFrontier, uint32_t numPrevFrontier, uint32_t* numCurrFrontier,  uint32_t level) {
...
void bfs_kernel(uint32_t numBlocks, uint32_t numThreadsPerBlock, CSRGraph csrGraph_d, uint32_t* nodeLevel_d, uint32_t* prevFrontier_d, uint32_t* currFrontier_d, uint32_t numPrevFrontier, uint32_t* numCurrFrontier_d, uint32_t level) {
    _bfs_kernel <<< numBlocks, numThreadsPerBlock >>> (csrGraph_d, nodeLevel_d, prevFrontier_d, currFrontier_d, numPrevFrontier, numCurrFrontier_d, level);
}
