...
int main(int argc, char** argv)
{
	...
	DPU_FOREACH(dpu_set, dpu)
	{
		...
		if (dpuNumNodes > 0) {
			...
			uint32_t dpuNodeLevel_m = mram_heap_alloc(&allocator,
			    dpuNumNodes * sizeof(uint32_t));
			uint32_t dpuVisited_m = mram_heap_alloc(&allocator,
			    numNodes / 64 * sizeof(uint64_t));
			uint32_t dpuCurrentFrontier_m = mram_heap_alloc(&allocator,
			    dpuNumNodes / 64 * sizeof(uint64_t));
			uint32_t dpuNextFrontier_m = mram_heap_alloc(&allocator,
			    numNodes / 64 * sizeof(uint64_t));
	...
	if (status) {
		dfatool_printf("[::] BFS-UMEM | n_dpus=%d n_ranks=%d n_tasklets=%d e_type=%s n_elements=%d "
		...
		dfatool_printf(" latency_alloc_us=%f latency_load_us=%f latency_write_us=%f latency_kernel_us=%f latency_sync_us=%f latency_read_us=%f latency_cpu_us=%f latency_free_us=%f\n",
